...
(function(){
 ...
 function loadArticle(card){
  ...
  return fetch(card.getAttribute('href'),{credentials:'same-origin'}).then(function(r){return r.text();}).then(function(t){
   ...
   var c=d.querySelector('.xans-board-read .detail')||d.querySelector('.fr-view-article')||d.querySelector('.xans-board-read');
   var imgs=[]; var html='';
...
(function(){
 ...
 function loadReviews(sec){
  ...
  var reviews=rows.map(function(r){ var a=r.querySelector('a[href*="/article/"]'); var st=r.querySelector('img[alt$="점"]'); var tds=[].slice.call(r.querySelectorAll('td')); var dIdx=-1; for(var k=0;k<tds.length;k++){ var tx=(tds[k].textContent||'').trim(); if(tx.length>=8 && tx.split('-').length>=3 && tx.indexOf(':')>-1){ dIdx=k; break; } } var rawDate= dIdx>-1 ? (tds[dIdx].textContent||'').trim() : ''; var writer= dIdx>1 ? (tds[dIdx-1].textContent||'').trim() : ''; var dateF= rawDate ? rawDate.slice(0,16).split('-').join('.') : ''; return {href:a.getAttribute('href').split('?')[0], rating:st?parseInt(st.alt,10):5, title:(a.textContent||'').trim(), writer:writer, date:dateF, body:'', imgs:[]}; });
  sec.__rvwLoad=Promise.all(reviews.map(function(rv){ return fetch(rv.href,{credentials:'same-origin'}).then(function(r){return r.text();}).then(function(t){
    var d=new DOMParser().parseFromString(t,'text/html'); var c=d.querySelector('.xans-board-read .detail')||d.querySelector('.fr-view-article');
  ...
  }).catch(function(){}); })).then(function(){ return {reviews:reviews, pname:pname}; });
